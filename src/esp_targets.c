...
typedef esp_loader_error_t (*read_spi_config_t)(void *ctx, uint32_t efuse_base, uint32_t *spi_config);
...
static esp_loader_error_t spi_config_esp32(void *ctx, uint32_t efuse_base, uint32_t *spi_config);
static esp_loader_error_t spi_config_esp32xx(void *ctx, uint32_t efuse_base, uint32_t *spi_config);
static esp_loader_error_t spi_config_unsupported(void *ctx, uint32_t efuse_base, uint32_t *spi_config);
...
esp_loader_error_t loader_detect_chip(void *ctx, target_chip_t *target_chip, const target_registers_t **target_data)
{
    ...
    if (esp_loader_get_security_info(ctx, &security_info) == ESP_LOADER_SUCCESS) {
    ...
    RETURN_ON_ERROR( esp_loader_read_register(ctx, CHIP_DETECT_MAGIC_REG_ADDR,  &magic_value) );
    ...
    // address.
    RETURN_ON_ERROR(esp_loader_read_register(ctx, ESP32P4_SPI_DATE_REG, &magic_value));
...
esp_loader_error_t loader_read_spi_config(void *ctx, target_chip_t target_chip, uint32_t *spi_config)
{
    const esp_target_t *target = &esp_target[target_chip];
    return target->read_spi_config(ctx, target->efuse_base, spi_config);
}

esp_loader_error_t loader_read_mac(void *ctx, const target_chip_t target_code, uint8_t *mac)
{
    ...
    RETURN_ON_ERROR(esp_loader_read_register(ctx, target->efuse_base + target->mac_efuse_offset, &part1));
    RETURN_ON_ERROR(esp_loader_read_register(ctx, target->efuse_base + target->mac_efuse_offset + sizeof(uint32_t), &part2));
...
static esp_loader_error_t spi_config_esp32(void *ctx, uint32_t efuse_base, uint32_t *spi_config)
{
    *spi_config = 0;
    ...
    uint32_t reg5, reg3;
    RETURN_ON_ERROR( esp_loader_read_register(ctx, efuse_word_addr(efuse_base, 5), &reg5) );
    RETURN_ON_ERROR( esp_loader_read_register(ctx, efuse_word_addr(efuse_base, 3), &reg3) );
...
static esp_loader_error_t spi_config_esp32xx(void *ctx, uint32_t efuse_base, uint32_t *spi_config)
{
    *spi_config = 0;
    ...
    uint32_t reg1, reg2;
    RETURN_ON_ERROR( esp_loader_read_register(ctx, efuse_word_addr(efuse_base, 18), &reg1) );
    RETURN_ON_ERROR( esp_loader_read_register(ctx, efuse_word_addr(efuse_base, 19), &reg2) );
...
bool encryption_in_begin_flash_cmd(void *ctx, const target_chip_t target)
{
    return esp_target[target].encryption_in_begin_flash_cmd;
}

target_chip_t target_from_chip_id(void *ctx, const uint32_t chip_id)
{
