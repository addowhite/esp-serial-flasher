...
static esp_loader_error_t check_response(void *ctx, const send_cmd_config *config);

esp_loader_error_t loader_initialize_conn(void *ctx, esp_loader_connect_args_t *connect_args)
{
    esp_loader_error_t err;
    int32_t trials = connect_args->trials;

    do {
        loader_port_start_timer(ctx, connect_args->sync_timeout);
        err = loader_sync_cmd(ctx);
        if (err == ESP_LOADER_ERROR_TIMEOUT) {
            if (--trials == 0) {
                return ESP_LOADER_ERROR_TIMEOUT;
            }
            loader_port_delay_ms(ctx, 100);
...
esp_loader_error_t loader_run_stub(void *ctx, target_chip_t target, size_t baudrate)
{
    esp_loader_error_t err;
    const esp_stub_t *stub = &esp_stub[target];

    // Increase baud rate for fast transfer
    err = esp_loader_change_transmission_rate(ctx, baudrate);
    if (err == ESP_LOADER_ERROR_UNSUPPORTED_FUNC) {
        return err;
    } else if (err != ESP_LOADER_SUCCESS) {
        return err;
    } else {
        err = loader_port_change_transmission_rate(ctx, baudrate);
    ...
    for (uint32_t seg = 0; seg < sizeof(stub->segments) / sizeof(stub->segments[0]); seg++) {
        err = esp_loader_mem_start(ctx, stub->segments[seg].addr, stub->segments[seg].size, ESP_RAM_BLOCK);
        if (err != ESP_LOADER_SUCCESS) {
            return err;
        }
        ...
        while (remain_size > 0) {
            ...
            err = esp_loader_mem_write(ctx, data_pos, data_size);
            if (err != ESP_LOADER_SUCCESS) {
                return err;
            }
            data_pos += data_size;
    ...
    // Decrease baud rate for "OHAI" message
    err = esp_loader_change_transmission_rate(ctx, 115200);
    if (err == ESP_LOADER_ERROR_UNSUPPORTED_FUNC) {
        return err;
    } else if (err != ESP_LOADER_SUCCESS) {
        return err;
    } else {
        err = loader_port_change_transmission_rate(ctx, 115200);
        if (err != ESP_LOADER_SUCCESS) {
            return err;
        }
    }
    ...
    err = esp_loader_mem_finish(ctx, stub->header.entrypoint);
    if (err != ESP_LOADER_SUCCESS) {
        return err;
    }
    ...
    uint8_t buff[4];
    size_t recv_size = 0;
    err = SLIP_receive_packet(ctx, buff, sizeof(buff) / sizeof(buff[0]), &recv_size);
    if (err != ESP_LOADER_SUCCESS) {
        return err;
...
esp_loader_error_t send_cmd(void *ctx, const send_cmd_config *config)
{
    ...
    RETURN_ON_ERROR(SLIP_send(ctx, (const uint8_t *)config->cmd, config->cmd_size));

    if (config->data != NULL && config->data_size != 0) {
        RETURN_ON_ERROR(SLIP_send(ctx, (const uint8_t *)config->data, config->data_size));
    }
    ...
    for (uint8_t recv_cnt = 0; recv_cnt < response_cnt; recv_cnt++) {
        RETURN_ON_ERROR(check_response(ctx, config));
    }
...
static esp_loader_error_t check_response(void *ctx, const send_cmd_config *config)
{
    ...
    do {
        RETURN_ON_ERROR(SLIP_receive_packet(ctx,
                                            buf,
    ...
    if (status->failed) {
        log_loader_internal_error(ctx, status->error);
        return ESP_LOADER_ERROR_INVALID_RESPONSE;
    }
